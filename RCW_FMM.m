...
%% FIELD DISTRIBUTION 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
SxR = reshape(rx,[NH,NH]);
SyR = reshape(ry,[NH,NH]); 
SzR = reshape(rz,[NH,NH]);
normFactorA = sum(src.esrc);

ERBBX = zeros(Nx,Ny);
ERBBY = zeros(Nx,Ny);
ERBBZ = zeros(Nx,Ny);

Me = (-floor(NH/2):floor(NH/2));
Ne = (-floor(NH/2):floor(NH/2));
    ...
    for y = 1:Ny
        for m = 1:NH
            for n = 1:NH
                ...
                xr = x*dx;
                yr = y*dy;
                ERBBX(x,y) = ERBBX(x,y) + SxR(m,n)*exp(-1j*(kx*xr + ky*yr));
                ERBBY(x,y) = ERBBY(x,y) + SyR(m,n)*exp(-1j*(kx*xr + ky*yr));
                ERBBZ(x,y) = ERBBZ(x,y) + SzR(m,n)*exp(-1j*(kx*xr + ky*yr));
            end
        end
    end
end
...
%% FIELD DISTRIBUTION 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
SxT = reshape(tx,[NH,NH]);
SyT = reshape(ty,[NH,NH]); 
SzT = reshape(tz,[NH,NH]); 
    ...
    for y = 1:Ny
        for m = 1:NH
            for n = 1:NH
                ...
                xr = x*dx;
                yr = y*dy;
                ETTX(x,y) = ETTX(x,y) + SxT(m,n)*exp(-1j*(kx*xr + ky*yr));
                ETTY(x,y) = ETTY(x,y) + SyT(m,n)*exp(-1j*(kx*xr + ky*yr));
                ETTZ(x,y) = ETTZ(x,y) + SzT(m,n)*exp(-1j*(kx*xr + ky*yr));
            end
        end
    end
end
...
subplot(231)
h = imagesc(xa, ya, real(ERBBX)/normFactor);
hh = get(h, 'Parent'); 
set(hh, 'YDir', 'normal'); 
title('REFLECTED FIELD-X \rightarrow', 'Fontsize', 14); 
axis equal tight
colormap('jet')
colorbar;

subplot(232)
h = imagesc(xa, ya, real(ERBBY)/normFactor);
hh = get(h, 'Parent'); 
set(hh, 'YDir', 'normal'); 
title('REFLECTED FIELD-Y \rightarrow', 'Fontsize', 14); 
axis equal tight
colormap('jet')
colorbar;

subplot(233)
h = imagesc(xa, ya, real(ERBBZ)/normFactor);
hh = get(h, 'Parent'); 
set(hh, 'YDir', 'normal'); 
title('REFLECTED FIELD-Z \rightarrow', 'Fontsize', 14); 
axis equal tight
colormap('jet')
colorbar;
...
subplot(234)
h = imagesc(xa, ya, real(ETTX)/normFactor);
hh = get(h, 'Parent'); 
set(hh, 'YDir', 'normal'); 
title('TRANSMITTED FIELD-X \rightarrow', 'Fontsize', 14);
axis equal tight
colormap('jet')
colorbar; 

subplot(235)
h = imagesc(xa, ya, real(ETTY)/normFactor);
hh = get(h, 'Parent'); 
set(hh, 'YDir', 'normal'); 
title('TRANSMITTED FIELD-Y \rightarrow', 'Fontsize', 14);
axis equal tight
colormap('jet')
colorbar; 

subplot(236)
h = imagesc(xa, ya, real(ETTZ)/normFactor);
hh = get(h, 'Parent'); 
set(hh, 'YDir', 'normal'); 
title('TRANSMITTED FIELD-Z \rightarrow', 'Fontsize', 14);
axis equal tight
colormap('jet')
colorbar; 
